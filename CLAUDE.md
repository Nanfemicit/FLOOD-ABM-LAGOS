...
- **Recession mechanism: implemented.** At the start of each `apply_rainfall()` call, before that day's rainfall is used, `water_depth` is reduced by up to `drain_mm` per cell (floored at zero, applied uniformly including at pits, drained amount added to `exited_total_mm_cells`) — sequenced *before* that day's rainfall/routing/cap pipeline specifically so `drain_mm` isn't spent twice against the same day's fresh water. Re-ran the June 2-4 comparison: flooded 0.98% (well) vs 15.82% (poor), built-up core 4.28% vs 68.14%. The recession itself is now visible and tells its own story: under well-maintained drainage, 26.7% of cells that peaked flooded during the 3-day event had already receded below threshold by day 3; under poor maintenance, only 0.01% had — meaning poor drainage doesn't just flood more, it keeps what floods underwater far longer, which is its own distinct piece of the human-cost argument (duration, not just occurrence). Not yet resolved, deferred rather than solved:
  - Evaporation as a separate loss term (recession here is drainage-only).
  - Re-mobilizing already-settled water back into the flow network (once water settles, whether via a pit or the recession drain, it never resumes moving downhill — it can only shrink in place).
  - `exited_total_mm_cells` currently combines two different physical fates into one counter — water that exited via surface routing (a flat/water boundary or off-domain edge) and water that drained out of standing depth via the new recession step — worth splitting into two counters later if the distinction ever matters for reporting.
  - The per-hop outflow cap in the routing loop technically resets every hop, not every day, so a busy cell touched by many hops in one day's convergence could in principle convey more than one day's worth of `drain_mm` in total. Worth checking whether this is ever materially inflating a cell's effective daily throughput — not urgent given what's been observed so far (hop counts stayed low, 9-39, in every scenario tested).
