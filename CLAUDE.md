...
- **Known v1 simplification worth revisiting: no conveyance-capacity limit on routed water.** Routing in `model/flood_model.py` runs to full convergence with no cap on how much water a non-pit cell can pass along per day, so standing water can currently only accumulate at literal topographic pits — confirmed empirically in the June 2-4 daily smoke test, where flooded cells came out 100% pits. That's a real gap: undersized or blocked drainage infrastructure is one of the most commonly cited real causes of Lagos flooding, not just low-lying terrain, and this model can't yet represent a chokepoint backing up water on its own. Not fixed now — flagged for whenever drainage-capacity realism becomes the priority.
